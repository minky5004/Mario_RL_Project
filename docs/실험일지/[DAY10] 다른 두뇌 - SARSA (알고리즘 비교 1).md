...
**이번** · 두뇌를 교체 가능하게 `Brain` 인터페이스로 빼고(`-Dmario.algo`), 첫 다른 알고리즘으로 **SARSA**(트라이1)를 붙이고, 비교를 읽는 잣대로 **랜덤 바닥선**(트라이2)을 세운다. (다음 두뇌 — Expected SARSA·Monte Carlo·진화 — 부터는 [[DAY11]](./[DAY11]%20다른%20두뇌%20-%20Expected%20SARSA%20(알고리즘%20비교%202).md)처럼 알고리즘별 독립 일지로.)
...
> ▶ 비교 챕터의 다음 두뇌부터는 **알고리즘별 독립 일지**로 쌓는다. 세 번째 두뇌 Expected SARSA: [[DAY11] Expected SARSA](./[DAY11]%20다른%20두뇌%20-%20Expected%20SARSA%20(알고리즘%20비교%202).md)
