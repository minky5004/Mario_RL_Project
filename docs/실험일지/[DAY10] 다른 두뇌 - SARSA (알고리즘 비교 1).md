...
## 트라이 3 — Expected SARSA (on-policy, 기댓값판)
...
- 단 ε이 0.01로 작아지면 `(1-ε)·max + ε·mean ≈ max` = Q-Learning과 거의 같아진다. 후반이 거의 greedy인 이 환경에선 **셋 다 본선 무승부**일 가능성이 높다 — 그래도 **바닥선(Random) 위로는 분리**돼야 진짜 학습(트라이2 잣대).
