...
**배경** · 표 Q-Learning은 1-1 클리어로 목표 달성, 더 튜닝해도 분산에 묻힌다(DAY9). Python은 *누가 행동을 정하는지* 모르므로 **Java 두뇌만 갈아끼우면** 같은 환경에서 비교 가능 → 새 챕터 = "여러 두뇌 대결." 이번엔 `Brain` 인터페이스 분리(`-Dmario.algo`) + **SARSA**(트라이1) + **랜덤 바닥선**(트라이2). 다음 두뇌부터는 알고리즘별 독립 일지.
