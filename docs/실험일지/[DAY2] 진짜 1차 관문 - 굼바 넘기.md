...
> **DAY1에서 남은 숙제**: 500판을 돌려도 마리오는 ~x290에서 반복해 죽었다 (baseline 상세: [\[DAY1\] 최초세팅](<[DAY1] 최초세팅.md>)).
> 이 문서는 그 **x290의 벽을 한 번에 하나씩** 넘어보는 기록이다.
...
> 트라이 1에서 **진짜 1차 벽은 굼바(x290)** 로 밝혀졌다. 굼바를 넘는 법은 점프(밟기 또는 뛰어넘기)이고,
> 상태에는 이미 적 신호(`enemy_near`)가 들어 있다. **그런데도 x290에서 반복해 죽는다.**
> 그래서 곧장 무언가를 바꾸기 전에 **"왜 못 넘는지"를 먼저 진단**하고, 그 결과에 맞춰 한 가지만 바꾼다.
...
> 트라이 2에서 픽셀 휴리스틱이 **마리오를 굼바로 오인**함이 드러났다. 추측을 버리고
> nes-py 의 `env.ram` 에서 **적의 실제 좌표를 직접 읽어** 마리오와의 거리를 계산한다.
