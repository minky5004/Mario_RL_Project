...
> 🎯 **발밑의 함정 — 구덩이 거리 인식**: 굼바에서 통한 패턴을 발밑 지형에 복제한다.

**배경** · DAY3에서 첫 굼바는 거의 넘었으나(후반 93%) 병목이 뒤로 밀렸다 — 새 벽:

| 2차 굼바 (x720~905) | 첫 구덩이 (x≈1120) |
|:-:|:-:|
| 24.6% 막힘 | 19.8% 막힘 |

(상세: [\[DAY3\] 인식에서 행동으로](<[DAY3] 인식에서 행동으로 - 굼바 밟기 보상.md>))

**이번** · 구덩이까지 거리를 숫자로 줘 피하게 한다(굼바 거리화의 복제).
...
**관찰** · DAY3에서 첫 굼바는 거의 넘었으나(후반 93%) 병목이 뒤로 밀려 2차 굼바·첫 구덩이(x≈1120)가 새 벽이 됐다.

**추론** · 굼바에서 "거리를 숫자로 줘 피하게" 한 패턴이 통했으니, 같은 방식을 발밑 지형(구덩이)에 복제하면 통할까. 단 픽셀이 두 번 속였으니 상태에 넣기 전에 측정으로 먼저 검증한다.

**결정** · 바닥 타일맵에서 앞 구덩이까지 거리를 단계화(`pit_dist`)해 상태에 넣는다(임시 로그 검증 후).
...
**관찰** · 트라이1에서 구덩이 거리는 진짜 신호였지만 통과율·낙사를 거의 못 바꿨다(도달판의 절반만 통과).

**추론** · 인식이 막혔으니 굼바에서 했듯 보상으로 행동을 강화해본다 — 구덩이를 무사히 건너면 보너스를 주면 통과를 더 학습할까(굼바 밟기 보너스와 대칭).

**결정** · 구덩이를 무사히 통과하면 +50을 준다.
