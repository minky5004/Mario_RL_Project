...
> 🎯 **상태가 아니라 엔진** — 예산·행동·알고리즘을 친다.

**배경** · DAY5까지 제어 상태 3종(해상도·높이·방향) 모두 구덩이 도달중 통과율 **~50% 벽**을 못 넘었다. "보상·인식·제어 상태로 제어 못 풂"이 확정됐다. (상세: [\[DAY5\] 둔한 발](<[DAY5] 둔한 발 - 프레임 스킵 줄이기.md>))

**범인** · 병목은 상태가 아니라 **엔진** — 학습 예산 / 행동 공간 / 알고리즘.

**이번** · 가장 싼 것부터 친다: ① 예산 → 안 되면 ② 행동 → 안 되면 ③ 알고리즘.
...
**관찰** · DAY5에서 제어 상태 3종(해상도·높이·방향)을 추가했는데도 구덩이 도달중 통과율이 셋 다 ~50% 벽을 못 넘었다.

**추론** · 병목은 상태가 아니다. 남은 후보(예산/행동/알고리즘) 중 예산은 코드 한 줄로 가장 싸고, 다른 후보를 시험하기 전에 "애초에 학습이 충분했나"라는 전제부터 정리해준다. DAY5가 분산일 수 있다는 의심도 판수를 늘리면 함께 해소된다.

**결정** · 가장 싸고 전제를 깔아주는 학습 예산(500 → 1500판)부터 친다.
